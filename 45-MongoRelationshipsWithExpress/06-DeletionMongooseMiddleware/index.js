...
app.get('/farms/:id', wrapAsync(async (req, res, next) => {
    const { id } = req.params;
    const farmObj = await Farm.findById(id).populate('products');
...
app.delete('/farms/:id', wrapAsync(async (req, res, next) => {
    ...
    const
     farm = await Farm.findByIdAndDelete(id);
...
app.post('/farms/:id/products', wrapAsync(async (req, res) => {
    ...
    productObj.farm = farmObj;
    farmObj.products.push(productObj);

    await productObj.save();
    await farmObj.save();

    res.redirect(`/farms/${farmObj._id}`);
}));
...
app.get('/products/:id', wrapAsync(async (req, res, next) => {
    const { id } = req.params;
    const productObj = await Product.findById(id).populate('farm', 'name');
